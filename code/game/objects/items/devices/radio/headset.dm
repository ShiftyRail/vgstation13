...
/obj/item/device/radio/headset/talk_into(datum/speech/speech_orig, channel=null)
	if(!broadcasting)
		return
	if(usr?.client?.prefs.get_pref(/datum/preference_setting/enum/headset_sound))
...
/obj/item/device/radio/headset/receive_range(freq, level)
	if(ishuman(src.loc))
		...
		if(H.ears == src)
			if(H.client && (H.client.prefs.get_pref(/datum/preference_setting/enum/headset_sound) == HEADSET_SOUND_ALL))
