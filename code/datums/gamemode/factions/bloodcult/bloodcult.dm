	...
	for(var/marker in holomap_markers)
		...
		if(holomarker.z == map.zMainStation && holomarker.filter & HOLOMAP_FILTER_CULT)
			if(map.holomap_offset_x.len >= map.zMainStation)
				markerImage.pixel_x = 2*(holomarker.x-8+map.holomap_offset_x[map.zMainStation])
				markerImage.pixel_y = 2*(holomarker.y-8+map.holomap_offset_y[map.zMainStation])
			else
				markerImage.pixel_x = 2*(holomarker.x-8)
				markerImage.pixel_y = 2*(holomarker.y-8)
