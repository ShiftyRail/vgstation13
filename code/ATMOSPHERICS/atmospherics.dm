...
/obj/machinery/atmospherics/update_icon(var/adjacent_procd,node_list)
	...
	for (var/missing_dir in missing_nodes)
		...
		switch (missing_dir)
			if (NORTH)
				nodeex.pixel_y = 2*ex_node_offset

			if (SOUTH)
				nodeex.pixel_y = -2*ex_node_offset

			if (EAST)
				nodeex.pixel_x = 2*ex_node_offset
...
/obj/machinery/atmospherics/proc/setPipingLayer(new_layer = PIPING_LAYER_DEFAULT)
	...
	pixel_x = 2*(piping_layer - PIPING_LAYER_DEFAULT) * PIPING_LAYER_P_X
	pixel_y = 2*(piping_layer - PIPING_LAYER_DEFAULT) * PIPING_LAYER_P_Y
	update_planes_and_layers()
